...
class Adb(object):
    ...
    def start_server(self):
        ...
        """
        cmd = ['tools/adb', 'start-server']
        subprocess.call(cmd)
        # checking the emulator state
        cmd = ['tools/adb', self.device, 'get-state']
        ...
        if state.find('device') == -1:
            ...
            cmd = ['tools/adb', 'connect', self.service]
    ...
    def exec_out(args):
        ...
        """
        cmd = ['tools/adb', Adb.device, 'exec-out'] + args.split(' ')
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    ...
    def shell(args):
        ...
        Args:
            args (string): Command to execute.
        """
        cmd = ['tools/adb', Adb.device, 'shell'] + args.split(' ')
